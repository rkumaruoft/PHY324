...
if __name__ == "__main__":
    ...
    ax.plot(t_numeric, position_fit, color="red", label="Fit Line")
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig("Graphs/counter_fit.png", dpi=200)
    plt.show()
    ...
    print("Period (T) = ", popt[2], "err=", np.sqrt(pcov[2][2]))
    print("Wave Phase (phi) = ", popt[3], "err=", np.sqrt(pcov[3][3]))
    print("Equilibrium angle for Counter (base)= ", popt[4], "err=", np.sqrt((np.sqrt(pcov[4][4]) ** 2)))

    plt.xlabel("Time (seconds)")
    plt.ylabel("Mirror Angle (radians)")
    plt.grid(True)
    plt.legend()
    plt.savefig("Graphs/counter_angle_vs_time.png", dpi=200)
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("Graphs/counter_residuals.png", dpi=200)
    plt.show()
    ...
    # Equilibrium position for Counter(base)=  0.05224054844134875 err= 0.0026950257328868994
    ...
    theta_equil_counter = popt[4]
    theta_equil_counter_err = np.sqrt((np.sqrt(pcov[4][4]) ** 2))
    pos_equil_counter = 0.05224054844134875
    pos_equil_counter_err = 0.0026950257328868994
    ...
    # Period from fit
    T = 305.5971833288585  # seconds
    T_err = np.sqrt(0.06266935005308796 ** 2 + (1 / np.sqrt(12)) ** 2)  # seconds
    print("T", T, T_err)

    # **Method 1: Using theta equilibrium difference**
    theta_method1 = abs(theta_equil_counter - theta_equil_empty)
    theta_err_method1 = theta_equil_counter_err

    # **Method 2: Using position equilibrium difference**
    s = abs(pos_equil_counter - pos_equil_empty)
    s_err = max(pos_equil_counter_err, pos_equil_empty_err)
    print("S", s, s_err)
