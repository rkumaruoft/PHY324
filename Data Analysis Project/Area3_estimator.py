...
for ievt in range(1000):
    ...
    area3[ievt] = np.sum([x for x in current_data[1000:1180]])

area3 *= 1000  # convert from V to mV

num_bins1 = 60
# bin_range1 = (min(area3), max(area3))
bin_range1 = (20, 37)
print(bin_range1)
"""
...
popt1, pcov1 = curve_fit(myGauss, bin_centers1, n1,
                         sigma=sig1, p0=(40, 28, 0.2, 5), absolute_sigma=True)
...
fontsize = 14
plt.text(20, 40, r'$\mu$ = %3.2f keV' % (popt1[1]), fontsize=fontsize)
plt.text(20, 35, r'$\chi^2$/DOF=', fontsize=fontsize)
plt.text(20, 30, r'%3.2f/%i' % (chisquared1, dof1), fontsize=fontsize)
plt.text(20, 25, r'$\sigma$ = %3.2f keV' % (popt1[2]), fontsize=fontsize)
plt.text(20, 20, r'$\chi^2$ prob.= %1.1f' % (1 - chi2.cdf(chisquared1, dof1)), fontsize=fontsize)
plt.savefig("Plots/area3_pre.png", dpi=200)
plt.show()
...
plt.legend(loc='upper right')
fontsize = 14
plt.text(7.3, 40, r'$\mu$ = %3.2f keV' % (popt1[1]), fontsize=fontsize)
plt.text(7.3, 35, r'$\chi^2$/DOF=', fontsize=fontsize)
plt.text(7.3, 30, r'%3.2f/%i' % (chisquared1, dof1), fontsize=fontsize)
plt.text(7.3, 25, r'$\sigma$ = %3.2f keV' % (popt1[2]), fontsize=fontsize)
plt.text(7.3, 20, r'$\chi^2$ prob.= %1.1f' % (1 - chi2.cdf(chisquared1, dof1)), fontsize=fontsize)
plt.savefig("Plots/area3_post.png", dpi=200)
plt.show()
"""Residuals"""
...
plt.legend()
plt.savefig("new_plots/area3_residuals", dpi=200)
plt.show()
